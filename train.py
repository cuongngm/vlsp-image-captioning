...
def train(args, train_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer, epoch, convert):
    """
...
def validate(args, val_loader, encoder, decoder, criterion, convert):
    ...
    with torch.no_grad():
        ...
        for i, (imgs, caps, caplens) in enumerate(val_loader):
            ...
            print('caplens', caplens.size())
            """
            # Move to device, if available
            imgs = imgs.to(device)
            caps = caps.to(device)
            caplens = caplens.to(device=device)
